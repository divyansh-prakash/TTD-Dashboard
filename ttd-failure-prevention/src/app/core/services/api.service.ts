...
import { ByPlatformResponse, FilterOptions, FailureQueueFilters, TrendResponse, PlatformDetailResponse, PlatformSummaryResponse, ContentHitsResponse, PeriodComparison, SegmentRankingsResponse, SegmentDetail, PlatformSegmentItem, PubmaticSummary, PubmaticAppidBreakdown, PubmaticContentCoverageRow, PubmaticContentGapRow, PubmaticMatchbyRow } from '../models/failure-queue.model';
...
export class ApiService {
  ...
  private addCommonFilters(params: HttpParams, filters: Partial<FailureQueueFilters>): HttpParams {
    if (filters.region  && filters.region  !== 'all') params = params.set('region',  filters.region);
    if (filters.partner && filters.partner !== 'TTD') params = params.set('partner', filters.partner);
    return params;
  }
  ...
  getByPlatform(filters: Partial<FailureQueueFilters>, offset = 0, limit = 25): Observable<ByPlatformResponse> {
    ...
    params = this.addCommonFilters(params, filters);
  ...
  getPubmaticSummary(filters: Partial<FailureQueueFilters>): Observable<PubmaticSummary> {
    let params = new HttpParams();
    if (filters.dateFrom) params = params.set('dateFrom', filters.dateFrom);
    if (filters.dateTo)   params = params.set('dateTo',   filters.dateTo);
    params = this.addCommonFilters(params, filters);
    return this.http.get<PubmaticSummary>(`${this.base}/failure-queue/pubmatic-summary`, { params });
  }

  getPubmaticAppidBreakdown(filters: Partial<FailureQueueFilters>): Observable<PubmaticAppidBreakdown> {
    let params = new HttpParams();
    if (filters.dateFrom) params = params.set('dateFrom', filters.dateFrom);
    if (filters.dateTo)   params = params.set('dateTo',   filters.dateTo);
    params = this.addCommonFilters(params, filters);
    return this.http.get<PubmaticAppidBreakdown>(`${this.base}/failure-queue/pubmatic-appid-breakdown`, { params });
  }

  getPubmaticMatchbyBreakdown(filters: Partial<FailureQueueFilters>): Observable<PubmaticMatchbyRow[]> {
    let params = new HttpParams();
    if (filters.dateFrom) params = params.set('dateFrom', filters.dateFrom);
    if (filters.dateTo)   params = params.set('dateTo',   filters.dateTo);
    params = this.addCommonFilters(params, filters);
    return this.http.get<PubmaticMatchbyRow[]>(`${this.base}/failure-queue/pubmatic-matchby-breakdown`, { params });
  }

  getPubmaticContentGap(filters: Partial<FailureQueueFilters>): Observable<PubmaticContentGapRow[]> {
    let params = new HttpParams();
    if (filters.dateFrom) params = params.set('dateFrom', filters.dateFrom);
    if (filters.dateTo)   params = params.set('dateTo',   filters.dateTo);
    params = this.addCommonFilters(params, filters);
    return this.http.get<PubmaticContentGapRow[]>(`${this.base}/failure-queue/pubmatic-content-gap`, { params });
  }

  getPubmaticContentCoverage(filters: Partial<FailureQueueFilters>): Observable<PubmaticContentCoverageRow[]> {
    let params = new HttpParams();
    if (filters.dateFrom) params = params.set('dateFrom', filters.dateFrom);
    if (filters.dateTo)   params = params.set('dateTo',   filters.dateTo);
    params = this.addCommonFilters(params, filters);
    return this.http.get<PubmaticContentCoverageRow[]>(`${this.base}/failure-queue/pubmatic-content-coverage`, { params });
  }
